...
import { getAWSKey, getFileIcon } from '../../utils/function';
...
const Header = props => {
    const { access, file, detailsPanelOpen } = props;
    ...
    const download = useCallback(async () => {
        ...
        try {
            ...
            const response = access
                ? await axios.get(`/open/files/download/${id}?key=${key}`, { responseType: 'blob' })
                : await axios.get(`/file/download?key=${key}`, { responseType: 'blob' });
    ...
    }, [id, file, errorHandler, showResponse, access]);
                ...
                <Grid container alignItems='center' pacing={1}>
                    <Grid item xs={6} md={10}>
                        <Box
                            ...
                            <Image name='logo-text-light.png' sx={{ height: '25px' }} />
                        </Box>
                    </Grid>
                    ...
                    <Grid item xs>
                        <Stack direction='row' alignItems='center' spacing={1}>
                            <Icon
                                name={file?.mimetype ? getFileIcon(file.name) : 'file.png'}
                                height='25px'
                            />
                            <Typography
                                variant='h6'
                    ...
                    <Grid item>
                        <Stack direction='row' alignItems='center' justifyContent='flex-end' mt={1}>
